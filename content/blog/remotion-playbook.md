...
I watched a long interview this week about a system where AI agents recreate brand videos in Remotion. The kind of motion graphics ElevenLabs or Higgsfield ship, rebuilt by Claude Code in about fifteen minutes, no After Effects and no keyframes. It was one of the best breakdowns I have seen. And when it ended, there was nothing to download. No scripts, no prompts, just a promise to maybe open source it if enough people asked in the comments. People asked. It is still not on GitHub.
...
The idea is honest about what each model is good at. Claude Code writes excellent animation code for videos it cannot watch, which takes a certain confidence. Gemini can actually watch video, and it happens to write the cleanest JavaScript animation logic of any model right now. So the pipeline splits the work.
...
I also wrote down the failure everyone hits. When an image sits behind text, agents reliably build them side by side instead of stacked. Two graders, a quality gate, a learnings file, and the logo still ends up next to the headline instead of under it. So the scene reader prompt now demands explicit z-order for every element, and the rubric checks it twice.
